...
const Hero = ({
  ...
  if(dataHero){ //es el que va a quedar...
    const {title,title_color,text,text_color,text_position,line_position,line_color,image,layout,align} = dataHero;
    return (
      <section className={
          `${
            styles.heroSection
        ...
        style={{
          backgroundImage: layout !== 'poster' ? `url(${image['super-large']})` : '',
        ...
        <div
          ...
          {title && (
            <h1
              ...
              dangerouslySetInnerHTML={{__html:text_position==='before'?`<span style='color:${text_color}'>${text}</span>${title}`:title}}
            />
          )}
          {line_color && (
            <div
